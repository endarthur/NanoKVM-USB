...
```
1. Target PC opens sender page in browser
...
### Sender Page (Target PC)

Create `sender.html` to run on target PC:
...
2. Open on target PC: firefox sender.html
...
async function connectViaPeerJS() {
    ...
    try {
        ...
        console.log('Connected to target PC!');
...
async function setupPeerJSAutoConnect() {
    ...
    peer.on('open', async (id) => {
        ...
        // 3. HID device auto-types sender URL on target PC
        // 4. Target PC opens sender page automatically
...
3. Cardputer auto-types sender URL on target PC
